...
Binit=( (1- perturb/2) + perturb*intpnoise(:,1) );%.*( 1 - perturb*cos(k0x*x) );
Sinit=( (1- perturb/2) + perturb*intpnoise(:,2) );%.*( 1 - perturb*cos(k0x*x) );
Hinit=( (1- perturb/2) + perturb*intpnoise(:,3) );
...
while tlast<tmax && Bdiff>1e-10
        ...
        if mod(tlast/Tyear,10)<1
            ...
            Bmin=min(Ulast(2*Nx+(1:Nx)));
            Bmax=max(Ulast(2*Nx+(1:Nx)));
            Bdiff=Bmax-Bmin;
...
box(axS,'on');
hold(axS,'all');
imagesc(x,t/Tyear,S'), caxis(axS,[0,Suv]), colormap(axS,flipud(parula));
set(gca,'YDir','normal')
ylim([min(t)/Tyear,tmax/Tyear])
xlim([0,Lx])
xlabel('x (m)')
...
box(axH,'on');
hold(axH,'all');
imagesc(x,t/Tyear,H'), caxis(axH,[0,Huv]), colormap(axH,flipud(parula));
set(gca,'YDir','normal')
ylim([min(t)/Tyear,tmax/Tyear])
xlim([0,Lx])
xlabel('x (m)')
...
subplot(3,1,2),hold on, cla,...
    ...
    plot(x,Suv*ones(size(x)),'blue','LineWidth',1,'LineStyle',':'),...    
    ylabel('S'),xlim([0,Lx]),ylim([0,3*Suv]),pbaspect([8 1 1])
subplot(3,1,3),hold on, cla, ...
    plot(x,H(:,nn),'cyan','LineWidth',1),...
    plot(x,Huv*ones(size(x)),'cyan','LineWidth',1,'LineStyle',':'),...    
    ylabel('H (cm)'),xlim([0,Lx]),ylim([0,25*Huv]),pbaspect([8 1 1])
